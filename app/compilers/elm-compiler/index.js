...
import { createTokenHash, cleanUpExpression } from './helpers'
...
function getFormattedError(error) {
    const errorStringPrefix =
        'Error: Command failed: cd temp && elm-make Main.elm --output=main.js && node main.js'
    const errorString = error.toString()
    const correctedErrorString = errorString
        .split('\n')
        .map(str => {
            if (str.match(/^\d+\|/)) {
...
function writeSourcesToElmPackageJson(
    templateFileContents,
    basePathForEditorCode
) {
    const packageJsonFilePath = `${tempFolderPath}/elm-package.json`

    let packageJsonFileContents = {
        ...templateFileContents,
        'source-directories': _.uniq(
            templateFileContents['source-directories'].concat([
                path.resolve(tempFolderPath),
                path.resolve(basePathForEditorCode)
            ])
        )
    }
    ...
    if (basePathForEditorCode !== path.resolve(tempFolderPath)) {
        ...
        while (true && depth < maxDepth) {
            ...
            if (_.includes(filesInFolderToCheck, 'elm-package.json')) {
                const tempPackageJsonContent = jsonfile.readFileSync(
                    `${folderToCheck}/elm-package.json`
                )
                const sourceDirectories =
                    tempPackageJsonContent['source-directories']

                packageJsonFileContents = {
                    ...packageJsonFileContents,
                    'source-directories': _.uniq(
                        packageJsonFileContents['source-directories'].concat(
                            _.trimEnd(
                                `${folderToCheck}/${sourceDirectories}`,
                                '/.'
                            )
                        )
                    )
                ...
                folderToCheck =
                    _.initial(folderToCheck.split('/')).join('/') || '/'
            }
        }
    }
...
function updateFileSources(openFilePath = tempFolderPath) {
    if (
        (openFilePath && lastOpenFilePath === openFilePath) ||
    ...
    return writeSourcesToElmPackageJson(
        packageJsonTemplateFileContents,
        openFilePath || tempFolderPath
    )
}
...
function writeCodeToFile(code) {
    ...
    if (code.startsWith('module ')) {
        ...
        codeToWrite = code.replace(
            `module ${inlineModuleName}`,
            'module UserCode'
        )
    } else if (code.trim() === '') {
        // if code panel is empty, insert a random function
        codeToWrite = `module ${moduleName} exposing (..)\n\nrandomIdentityFunction x = x`
    } else {
        codeToWrite = `module ${moduleName} exposing (..)\n\n${code}`
    }

    return writeFile(`${codePath}/${moduleName}.elm`, codeToWrite).then(
        () => moduleName
    ) // return the moduleName for playgroundFileWritercatch((err) => console.log('error writing file', `${codePath}/${moduleName}.elm`, err.toString()))
}
        ...
        .map(command => {
            if (command.value.trim().length === 0) {
                ...
                const newLines = _.times(command.newlines, _.constant('\n'))
                    .map(() => '"""\n"""')
                    .join(',')
                return `Basics.toString (${cleanUpExpression(
                    command.value
                )}),${newLines}`
            }
        })
...
function getGeneratedFrolicFileContent(
    expression,
    importStatements,
    statements,
    userModuleName
...
function getGeneratedMainFileContent(
    expression,
    importStatements,
    statements,
    userModuleName
...
import ${userModuleName} exposing (..)`
    ...
    if (expression.type === 'frolicExpression') {
        fileContent = getGeneratedFrolicFileContent(
            expression,
            importStatements,
            statements,
            userModuleName
...
function writeFilesForExpressions(tokens, userModuleName, codePath) {
    ...
    const statements = tokens
        ...
        .map(token => token.value)
        .join('\n')
    const allExpressions = tokens.filter(
        token =>
            token.type === 'expression' ||
            token.type === 'renderExpression' ||
            token.type === 'frolicExpression'
    )
    const expressions = allExpressions.filter(notInCache)

    const fileWritePromises = expressions.map((expression, index) =>
        writeFile(
            `${codePath}/F${expression.hash}.elm`,
...
function getSource(module, expression) {
    ...
    if (!cachedSources[expression.hash]) {
        cachedSources[expression.hash] = fs
            .readFileSync(`${codePath}/${fileName}.js`)
            .toString()
    } else {
...
export function compile(code = '', playgroundCode = '', openFilePath) {
    ...
    return updateFileSources(openFileFolderPath)
        ...
        .then(userModuleName =>
            writeFilesForExpressions(tokensWithHashes, userModuleName, codePath)
        )
        .then(expressions => {
            // eslint-disable-line arrow-body-style
            return new Promise(() => {
                const allPromises = expressions
                    .filter(notInCache)
                    .map(expression => {
                        const fileName = `F${expression.hash}`
                        return promisifiedExec(
                            `cd ${codePath} && elm-make --yes ${fileName}.elm --output=${fileName}.js`
                        )
                    })
                ...
                elmMakePromises = new Promise((resolve, reject, onCancel) => {
                            ...
                            (err, resultList) => {
                                if (err) {
                                    ...
                                    resultList.forEach(process => {
                                        ps.kill(
                                            process.pid,
                                            errorGettingProcessInfo => {
                                                if (errorGettingProcessInfo) {
                                                    console.error(
                                                        'Error killing process ',
                                                        errorGettingProcessInfo.toString()
                                                    ) // eslint-disable-line no-console
                                                }
                                            }
                                        )
                    ...
                    return Promise.all(allPromises)
                        ...
                        .then(() => {
                            ...
                            sources = expressions.map((expression, index) =>
                                getSource(module, expression, index)
                            )

                            const elmComponents = sources.map(
                                (source, index) => {
