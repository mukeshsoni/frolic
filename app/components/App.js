...
function getSavedPreferences() {
  return getFromStorage('preferences').then(preferences => _.defaults(preferences, editorPrefsDefaults))
}
...
export default class App extends Component {
  ...
  componentWillMount() {
    ipcRenderer.on('menuActions', (event, message) => {
      ...
      case 'showPreferences':
        ...
        break
      default:
        console.log('don\'t understand the menu action', message.action) // eslint-disable-line no-console
      }
    })

    getSavedPreferences().then(preferences => this.setState({
      ...preferences
    }))
  }
  ...
  loadFileFromHistory() {
    getFromStorage('fileData').then((file) => {
      if (file.filePath) {
        readFile(file.filePath).then((content) => this.setState({ code: content.toString() }))
      }
    })
  }
  ...
  storeFilePathInDb() {
    return setToStorage('fileData', { filePath: this.state.openFilePath })
            .then(() => console.log('file path stored ', this.state.openFilePath)) // eslint-disable-line no-console
            .catch((err) => {
  ...
  handleFileSaveClick() {
    return saveFile(this.state.code, this.state.openFilePath, this.state.compiler.extensions).then((filePath) => {
      if (this.state.formatOnSave && this.state.compiler.formatCode) {
        this.state.compiler.formatCode(this.state.code).then((formattedCode) => {
          this.setState({
            code: formattedCode,
  ...
  handleGenerateTestClick() {
    if (this.state.compiler.generateTests) {
      ...
      alert('This compiler, ' + this.state.language + ', does not support test generation') // eslint-disable-line
    }
  }
  ...
  openPlaygroundFile() {
    return openFile(['frolic'])
            ...
            .catch((err) => console.log('error opening playground file', err.message)) // eslint-disable-line no-console
  }
  ...
  handleSavePlaygroundClick() {
    saveFile(this.state.playgroundCode, getPlaygroundFilePath(this.state.openFilePath, this.state.playgroundFilePath), ['frolic'], 'Save Playground')
      .then(() => this.compile())
      .catch((err) => console.log('error saving file ', err.message)) // eslint-disable-line no-console
  }
    ...
    .then(({ codeFile, playgroundFile }) => {
      this.setState({
        ...
        if (this.state.compiler.onNewFileLoad) {
          this.state.compiler.onNewFileLoad(this.state.openFilePath)
        }
